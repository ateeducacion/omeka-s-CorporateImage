...
Supported PHP: `>=7.4` (`composer.json`); check `config/module.ini` for Omeka compatibility.
...
Read [header-footer-settings](.agents/skills/header-footer-settings/SKILL.md) when you need to: change custom header/footer configuration or legacy settings compatibility.
Use the existing tests under `test/` for changed behavior; avoid new test frameworks.

## Verification

Run `make lint` and `make test` for PHP changes.
